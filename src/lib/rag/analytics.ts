...
/**
 * Statistieken voor het admin-dashboard. Aggregatie gebeurt in JS met een
 * redelijke bovengrens op het aantal rijen; bij grote volumes kan dit later
 * naar SQL-aggregatiefuncties verhuizen.
 */

// Indicatieve prijzen per model (dollar per 1 miljoen tokens) — alleen voor kostenschatting
...
export type Analytics = {
  ...
  gaps: number; // assistent-antwoorden zonder opgehaalde bron
  conversionRate: number; // % gesprekken dat tot een chatbot-lead leidde
  satisfactionRate: number; // % 👍 van alle feedback
  models: ModelUsage[];
  totalCostUsd: number;
  totalTokens: number;
};
...
/** Gegevens voor de pagina "feedback en onbeantwoorde vragen". */
export async function getReviewData(): Promise<ReviewData | null> {
  const supabase = getSupabaseAdmin();
  if (!supabase) return null;

  // Negatieve feedback met de bijbehorende antwoordtekst
  ...
  // Onbeantwoorde vragen: assistent-antwoorden zonder bron → de gebruikersvraag ervoor
  ...
  gapQuestions.reverse(); // nieuwste eerst
...
export async function getAnalytics(): Promise<Analytics | null> {
  ...
  // Assistent-berichten: tokens, model, kennisgaten
